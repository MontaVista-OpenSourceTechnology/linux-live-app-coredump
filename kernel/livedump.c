					  ...
					  unsigned long clone_flags)
{
	...
	clone->signal->oom_adj = param->oom_adj;
 ...
 */
void livedump_perform_dump(struct pt_regs *regs)
{
	...
	status = do_coredump(SIGKILL, SIGKILL, regs);
	/* Trying to fix the poor return value comes from underlying levels. */
	status = (status == 1 ? 0 : (status == 0 ? -EIO : status));
