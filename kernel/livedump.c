 ...
 *
 * Copyright 2013-2024 MontaVista Software LLC <source@mvista.com>
 *
			 ...
			 unsigned long clone_flags)
{
	...
	/* The task is being created, no need to worry about races here. */
	WRITE_ONCE(clone->__state, TASK_STOPPED);
	...
	if (clone_flags & CLONE_THREAD) {
		...
		refcount_inc(&dump->dumped_leader->sighand->count);
	...
	/* set_user_nice() can't run here, so just set the priority. */
	clone->static_prio = NICE_TO_PRIO(param->sched_nice);

	/* Clone will have inherited I/O scheduling class, but new priority. */
	prio = __get_task_ioprio(clone);
	if (prio < 0)
		return prio;
 ...
 */
static void livedump_perform_dump(kernel_siginfo_t *info,
				  struct livedump_context *dump)
{
	sigset_t blocked;
	long status;
 ...
 */
void __livedump_handle_signal(kernel_siginfo_t *info)
{
...
int do_livedump(struct task_struct *tsk, struct livedump_param *param)
{
	...
	/*
	 * Set this so alloc_pid() won't fail.  It doesn't matter what
	 * it is, it will never be used because we set pid_allocated
	 * below.
	 */
	dump->pid_ns->child_reaper = current;

	/*
	 * Make sure the namespace shutdown when pid_allocated < 3 is
	 * never hit, we will handle that ourself.
	 */
	dump->pid_ns->pid_allocated += 3;
