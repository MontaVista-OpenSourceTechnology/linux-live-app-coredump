...
static inline enum livedump_stage livedump_stage(struct livedump_context *dump)
{
	return dump->stage;
}
...
static inline bool task_in_livedump_stage(struct livedump_context *dump,
					  enum livedump_stage check_stage)
{
...
static inline void __livedump_send_sig(struct task_struct *tsk, bool force)
{
	...
	if (recalc_sigpending_tsk(tsk) && !force)
		signal_wake_up(tsk, 0);
	if (force)
					  ...
					  unsigned long clone_internal_flags)
{
	...
	} else if (clone_internal_flags & CLONE_INT_LIVEDUMP) {
...
extern void __livedump_handle_signal(kernel_siginfo_t *info);

static inline void livedump_handle_signal(kernel_siginfo_t *info)
{
